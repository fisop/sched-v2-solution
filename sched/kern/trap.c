...
// Contador global de ticks del timer.
...
trap_dispatch(struct Trapframe *tf)
{
	...
	case IRQ_TIMER:
		...
		// The tick is the kernel's unit of time: it measures time since
		// boot and is the basis for sys_sleep.
		ticks++;

		// Environments that finished sleeping go back to the ready
		// queue before the scheduling decision is made, so the
		// scheduler already considers them in this same round.
		sched_wakeup_sleeping();

		// Preemption: the CPU is yielded even if the environment never
		// called sys_yield.
		sched_yield();
		return;
	}
